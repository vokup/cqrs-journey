...
public class MessageReceiver : IMessageReceiver, IDisposable
{
    ...
    public MessageReceiver(DbContext dbContext, string name, string tableName)
        : this(dbContext, name, tableName, TimeSpan.FromMilliseconds(100))
    {
    }

    public MessageReceiver(DbContext dbContext, string name, string tableName, TimeSpan pollDelay)
    {
            ...
            using (var transaction = connection.BeginTransaction(IsolationLevel.ReadCommitted))
            {
                try
                {
                    ...
                    using (var command = connection.CreateCommand())
                    {
                        ...
                        command.Parameters.Add(new SqlParameter("@CurrentDate", SqlDbType.DateTime)
                        {
                            Value = currentDate
                        });
